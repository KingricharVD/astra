...
bool AstraDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule AstraDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t AstraDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t AstraDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t AstraDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t AstraDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool AstraDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void AstraDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void AstraDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void AstraDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void AstraDGP::createParamsInstance(){
...
dev::Address AstraDGP::getAddressForBlock(unsigned int blockHeight){
...
void AstraDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void AstraDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void AstraDGP::parseStorageOneUint64(uint64_t& value){
...
void AstraDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule AstraDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
