    ...
    m_selectedToken(0)
{
    ...
    ui->labelAmount->setToolTip(tr("The amount in ASTC20 Token to send."));
...
void SendTokenPage::on_numBlocksChanged(int newHeight)
{
    if(m_clientModel)
    {
        if (lastUpdatedHeight < newHeight) {
            ...
            ui->labelGasPrice->setToolTip(tr("Gas price: ASTRA/gas unit. Default = %1, Min = %2.").arg(
...
WalletModel::UnlockContext ctx(m_model->requestUnlock());
    ...
    {
        QString message = tr("To send %1 you need ASTRA in address <br /> %2.")
