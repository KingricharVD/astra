...
void HexAddressConverter::addressChanged(const QString& address) {
    ...
    if(!address.isEmpty()) {
        ...
        hasAstraAddress = IsValidDestination(addr);
        if (hasAstraAddress) {
            ui->label->setText("Address (Astra)");
        ...
        isAddressValid = hasHexAddress || hasAstraAddress;
    }
